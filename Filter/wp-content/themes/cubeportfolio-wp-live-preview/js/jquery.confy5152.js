    ...
    var pluginName = 'confy',
        ...
        pluginObject = {
                ...
                t.options.items.each(function(index, item) {
                    ...
                    switch (data.type) {
                        ...
                        case 'selectbox':
                            option = t._getValueFromString(data.option);
                            item.data('value', option);
                            item.find('option[data-value=' + option + ']').prop({
                                selected: true
                            });
                            break;
            ...
            showConfy: function(toggleButton, byUser) {
                ...
                if (byUser) {
                    localStorage.confyPlugin = JSON.stringify({
                        "closedByUser": "false"
                    });
                }
            },
            ...
            initEvents: function() {
                ...
                t.options.items.filter('[data-type="selectbox"]').on('change' + eventNamespace, function(e) {
                ...
                $('.cfy-item-add').on('click', function() {
                    var select = $(this).prev(),
                        next = select.find('option:selected').next(),
                        value = (next.length) ? next.val() : select.find('option').first().val();

                    select.val(value).prop('selected', true).change();
                });

                $('.cfy-item-sub').on('click', function() {
                    var select = $(this).next(),
                        prev = select.find('option:selected').prev(),
                        value = (prev.length) ? prev.val() : select.find('option').last().val();
